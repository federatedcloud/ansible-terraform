...
###################################################################
#resource "local_file" "internal" {
#  content = "${join("\n",google_compute_instance.mpi.*.network_interface.0.network_ip)}"
#  filename = "${path.module}/../ansible/hostfile"
#}
data "template_file" "mpi_hostfile" {
  template = "${file("mpi_hostfile.tmpl")}"
  vars {
   ip_addrs = "${join(",",google_compute_instance.mpi.*.network_interface.0.network_ip)}"
  }
}
...
resource "null_resource" "default" {
  ...
  connection {
    host = "${google_compute_instance.mpi.*.network_interface.0.access_config.0.nat_ip}"
  }
  provisioner "local-exec" {
